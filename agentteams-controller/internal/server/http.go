...
func NewHTTPServer(addr string, deps ServerDeps) *HTTPServer {
	...
	mux.Handle("GET /api/v1/workers/{name}/events", mw.RequireAuthz(authpkg.ActionGet, "worker", nameFn)(http.HandlerFunc(rh.GetWorkerEvents)))
	...
	mux.Handle("GET /api/v1/gateway/providers/{name}/models", mw.RequireAuthz(authpkg.ActionList, "gateway", nil)(http.HandlerFunc(gh.ListProviderModels)))
