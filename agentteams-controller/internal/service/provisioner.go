...
// injection HTTP endpoints to post operator-authored messages directly
// into a Manager's Admin DM room or a Team's leader room.
...
// when Config.SoloOperator is true: a single human is running AgentTeams alone,
...
func renderManagerWelcomeBodySolo(language, timezone string) string {
	return fmt.Sprintf(`This is an automated message from the AgentTeams setup. This is a fresh installation running in solo mode (single operator).
