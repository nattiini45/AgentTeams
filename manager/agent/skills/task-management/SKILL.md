...
- **Always use `manage-state.sh` to modify state.json** — never edit manually with jq. The script handles atomicity, deduplication, and initialization. When `agt` is installed it delegates to `agt manager-state`; set `HICLAW_MANAGER_STATE_IMPL=shell` to force the legacy shell implementation.
