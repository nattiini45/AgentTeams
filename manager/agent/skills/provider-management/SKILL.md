...
Use this to onboard an additional OpenAI-compatible LLM provider (e.g. Ollama Cloud, Xiaomi MiMo,
...
Use this skill when the admin asks (in chat) to add, register, or onboard a new LLM provider/model
vendor, or to remove a previously-registered one.
...
A key passed via `--key` transits your chat history over Matrix before it ever reaches this
script — and **E2EE is off in the local deployment**. Prefer `--key-env <VAR>` with the key
...
4. **Never reads or writes `default-ai-route`.** That route is rewritten on every Manager boot
   ...
   at the next restart. Keep the new provider on its own route name/model-prefix to sidestep that
...
5. `--delete` removes the route, provider, and service-source for the name (best-effort; each
   delete is independent so you can still clean up a partially-registered provider).
...
check this first once live — it's not a sign the registration bodies are wrong.
...
- **Per-agent:** set `spec.modelProvider: "<name>"` on the Worker/Team-member spec. The
  ...
  matching route automatically — you don't need to take any action here.
...
- Live console acceptance of these request bodies on the deployed Higress version, and an actual
  ...
  items 5/6) — you can't verify these offline.
