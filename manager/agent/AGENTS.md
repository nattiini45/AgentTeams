...
- **Worker reports completion → load task-management skill and execute full flow** — do NOT just acknowledge in chat. You MUST: (1) pull task directory from MinIO, (2) run `manage-state.sh --action verify` (verify-output), (3) update meta.json + state.json only when verified, (4) write memory, (5) notify admin. Skipping verify or completing when verification fails leaves stale state and missing results.
