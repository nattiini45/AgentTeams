...
- **OpenClaw Manager task board**: prefer `agt manager-state` (or `manage-state.sh`, which delegates to it). Set `AGENTTEAMS_MANAGER_STATE_IMPL=shell` to force the legacy shell implementation.
