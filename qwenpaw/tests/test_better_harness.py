"""Unit tests for the weekly better-harness trigger loop (qwenpaw worker)."""
...
from qwenpaw_worker.better_harness import (
...
def test_run_weekly_script_path() -> None:
    home = Path("/tmp/worker-a")
    assert _run_weekly_script(home) == home / "skills" / "better-harness" / "scripts" / "run-weekly.sh"
...
def test_loop_returns_immediately_when_disabled(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # Should return without looping; use a timeout to guard against a hang.
...
def test_loop_skips_when_script_missing(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async def _run_once_then_cancel() -> None:
        ...
        # Let the loop run one iteration (script missing -> debug skip), then cancel.
