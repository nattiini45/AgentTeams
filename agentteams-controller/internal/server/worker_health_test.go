...
func TestWorkerHealthProberUsesCache(t *testing.T) {
	...
	secret := &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{Name: "agentteams-creds-alpha", Namespace: "default"},
...
func TestGetWorkerRuntimeStatusIncludesHealthChecks(t *testing.T) {
	...
	secret := &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{Name: "agentteams-creds-alpha-dev", Namespace: "default"},
		Data:       map[string][]byte{"WORKER_GATEWAY_KEY": []byte("test-key")},
	}
	worker := &v1beta1.Worker{
...
func TestAttachHealthChecksBatchRespectsListBudget(t *testing.T) {
	...
	for i := 0; i < workerCount; i++ {
		...
		objects = append(objects,
			&corev1.Secret{
				ObjectMeta: metav1.ObjectMeta{Name: "agentteams-creds-" + name, Namespace: "default"},
				Data:       map[string][]byte{"WORKER_GATEWAY_KEY": []byte("test-key")},
			},
			&v1beta1.Worker{
