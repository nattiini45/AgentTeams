...
4. Read `last-session-snapshot.json` if it exists — instant operational context from last heartbeat (active tasks, escalations, worker health). If the snapshot is recent (< 1 hour), use it to prioritize what to check first. If stale or missing, fall back to normal state discovery.
