...
The `plan.md` shows:
- All project tasks and their status (`[ ]` pending / `[~]` in-progress / `[x]` completed)
- Which tasks are yours and what dependencies exist
- Links to task brief and result files for each task

## Workflow

1. When assigned a task in the project room, mark it started (see `task-progress`).
2. Execute the task following the normal task execution flow.
...
```bash
git config user.name "${AGENTTEAMS_WORKER_NAME}"
git config user.email "${AGENTTEAMS_WORKER_NAME}@hiclaw.local"
```
