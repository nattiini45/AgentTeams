"""Weekly better-harness self-review trigger for the managed worker process.
...
def _run_weekly_script(worker_home: Path) -> Path:
    # Builtin skills are pushed by the controller / Manager to the worker's
    # object-storage prefix (agents/<name>/skills/better-harness/) and mirrored
    # by the worker's sync into <worker_home>/skills/better-harness/. That
    # mirror root is the reliable location across restarts; the QwenPaw runtime
    # workspace (.qwenpaw/workspaces/default/skills/) is populated separately
    # from the agent package and is not guaranteed to hold this builtin skill.
    return worker_home / "skills" / "better-harness" / "scripts" / "run-weekly.sh"
...
async def run_better_harness_weekly_loop(
    ...
    *,
    ...
    script = _run_weekly_script(worker_home)
    logger.info(
