...
// forceSoloPeerMentions ensures every Team's PeerMentions is true when the
// controller runs in single-operator mode (AGENTTEAMS_SOLO_OPERATOR). With only
// one human in the org, cross-mentions between team members can never leak to a
// stranger, so always looping the operator in is strictly more useful than
// honoring an operator-unset false. When solo is false the Team is returned
// unchanged so spec-controlled PeerMentions (default true) is respected.
//
// The returned Team is a DeepCopy so the caller's original is never mutated —
// the reconciler may need the unmodified spec for drift comparison.
...
// leaderWorkerSpec derives the leader's effective WorkerSpec from the Team,
// merging team-wide and per-leader channel policies and inflating the
// GroupAllowExtra with peer worker runtime names + coordinator IDs + the
// team admin Matrix ID. Per-agent ModelProvider wins; an empty per-agent
// value falls back to the team-wide default.
...
// teamWorkerSpecToWorkerSpec derives a worker member's effective WorkerSpec
// from the Team + that worker's TeamWorkerSpec, applying the same policy
// merge as leaderWorkerSpec. When PeerMentions is true (the default), each
// worker's GroupAllowExtra is inflated with peer worker runtime names so
...
// memberSpecChanged reports whether a member's source spec has changed since
// its container was last provisioned, by comparing the stored SpecHash
// against a freshly-computed hash of the user-authored fields.
...
// hashMemberSourceSpec digests the user-authored fields that govern a member's
// container. Only fields the user explicitly types into the Team CR count;
// derived values (the team-wide peer list inflated into GroupAllowExtra, admin
// Matrix ID injections) are excluded. Leader hash covers Team.Spec.Leader +
// team-level ChannelPolicy + team ModelProvider; worker hash covers the
// matching TeamWorkerSpec + team-level ChannelPolicy + PeerMentions toggle +
// team ModelProvider. Returning "" on marshal error is safe (errside on
// recreation — see memberSpecChanged).
...
// leaderHeartbeatEvery returns the leader's configured heartbeat interval,
// or "" when heartbeat is disabled/unset.
...
// validateTeamRuntimeNames ensures no two team members share the same
// effective runtime workerName (which would alias Matrix/OSS identities).
...
// buildDesiredMembers builds the []MemberContext for a Team's leader + workers,
// merging team-wide and per-member channel policies, stamping controller-
// identity PodLabels, and computing per-member SpecChanged. The TeamReconciler
// calls this (via the legacy/decoupled paths) after resolving identities.
