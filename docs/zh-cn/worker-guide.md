...
`AGENTTEAMS_WORKER_NAME` 为必填（运行时身份 / 存储路径）。仅当 Worker CR 名称与该运行时身份不同时，才设置 `AGENTTEAMS_WORKER_CR_NAME`。
...
| 变量 | 说明 | 示例值 |
|------|------|--------|
| `AGENTTEAMS_WORKER_NAME` | 必填：Worker 运行时身份 | `alice` |
| `AGENTTEAMS_WORKER_CR_NAME` | 可选：当 Worker CR 名称与 `AGENTTEAMS_WORKER_NAME` 不同时的覆盖 | — |
